...
def compute_epf_gratuity_timeline(joining_date: str, as_of_date: str | None = None,
    ...
    # tenure_years (days/365.25) is an approximation used only for display —
    # eligibility booleans below compare exact calendar dates instead, since
    # the 365.25 approximation can put an employee's exact N-year anniversary
    # a hair under N when a leap day falls in the range (e.g. exactly 5
    # calendar years spanning one Feb 29 is 1826 days, and 1826/365.25 <
    # 5.0) — that mismatch would contradict the exact-date milestone list
    # below, which already used to be the only correct one.
    ...
    epf_tax_free_date = _add_years(joining, epf_tax_free_years)
    eps_pension_date = _add_years(joining, eps_pension_years)

    gratuity_vested = evaluation_date >= vesting_date
    if separation_date:
        gratuity = {
    ...
    epf_tax_free = evaluation_date >= epf_tax_free_date
    epf = {
    ...
    milestones = [
        ...
        {"label": f"EPF Withdrawal Tax-Free ({epf_tax_free_years} yrs)",
         "date": epf_tax_free_date.isoformat(), "rule_id": "EPF_TAX_FREE_WITHDRAWAL_YEARS",
        ...
        {"label": f"EPS Pension Eligibility ({eps_pension_years} yrs, if continuously transferred)",
         "date": eps_pension_date.isoformat(), "rule_id": "EPS_PENSION_MIN_SERVICE_YEARS",
         "status": "past" if evaluation_date >= eps_pension_date else "upcoming"},
    ]
